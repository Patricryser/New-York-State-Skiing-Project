...
nySkiers <- ((skiersRaw$northeast*nyShare)/51)*1000000
...
BelltempSkier <- cor(nySkiers,yearMean$maxTemp)
BelltempSkier

BellskiersTempReg <- lm(nySkiers~yearMean$maxTemp)
summary(BellskiersTempReg)

plot(yearMean$maxTemp,nySkiers)
abline(BellskiersTempReg,col = 'coral3', lwd=2)


#calculating the mean Tmax

BellmeanTmax <- mean(yearMean$maxTemp)
BellmeanTmax
...
NAO <- read.table('nao_index.txt', header = TRUE)
...
x1 <- mean(yearMean$maxTemp)+1.08
x1

x2 <- mean(yearMean$maxTemp)+1.98
x2
...
################################################################
confint_timestep <- function (x){
  seRegressionX0 <- matrix(NA,nrow=n,ncol=1)
  length(seRegressionX0)
  seRegressionX0[36]
  for (i in 1:36){
    seRegressionX0[i] <- sY*(sqrt(1-r2))*(sqrt((n-1)/(n-2)))*(sqrt(1+(1/n)+((x[i]-xBar)^2/((n-1)*((sX)^2)))))
  }
  
  plot(years,seRegressionX0,type='l')
  confidenceInterval95X0 <- seRegressionX0*2
  plot(years, confidenceInterval95X0, type='l', ylab='confidence interval (F)')
}
x0_all <- yearMean$maxTemp
confint_timestep(x0_all)
